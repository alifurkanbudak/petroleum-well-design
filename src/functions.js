function kNN(csvData, depth, x, y) {
  ...
  csvData.forEach((element) => {
    distances.push(
      Math.sqrt(
        Math.pow(parseInt(element.x) - parseInt(x), 2) +
          Math.pow(parseInt(element.y) - parseInt(y), 2)
      )
    );
  });
  ...
  for (let i of sortedDistanceInd) {
    if (
      parseInt(depth) - tol < parseInt(csvData[i].z) &&
      parseInt(csvData[i].z) < parseInt(depth) + tol
    ) {
      ind = i;
      break;
    }
